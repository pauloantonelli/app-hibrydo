...
export class PaoCenouraPage {
  ...
  public receitasIni: string = 'Clique em editar e escreva a técnica de preparo do ' + this.pagina[0];
  public conteudoCard;
  public textoEdicao;
  ...
  public chaveReceita: string = 'Tecnica de preparo ' + this.pagina[0];
  ...
  //PEGA OS VALORES RELATIVOS AOS INGREDIENTE DO BANCO
  valorIngrediente(chave: string) {
    return this.receitaProvider.verIngredientes(chave);
  }
  //PEGA OS VALORES RELATIVOS AOS INGREDIENTE DO BANCO
  valorReceita(chave: string) {
    return this.receitaProvider.verReceitas(chave);
  }
  //SALVA OS VALORES RELATIVOS AOS INGREDIENTE DO BANCO
  salvarIngrediente(chave: string, compras: Ingredientes) {
    this.receitaProvider.salvarIngredientes(chave, compras);
    this.mensagem('Atualizado com sucesso!');
  }
  //SALVA OS VALORES RELATIVOS AOS INGREDIENTE DO BANCO
  salvarReceita(chave: string, receita: string) {
  ...
  calculosValoresIngredientes(){
    ...
    var aux3 = this.valorCompras.cenoura * this.valorIngredientes.precoCenoura / Number(this.valorIngredientes.cenoura * 1000);
    ...
    var aux5 = this.valorCompras.goiabada * this.valorIngredientes.precoGoiabada / Number(this.valorIngredientes.goiabada * 1000);
